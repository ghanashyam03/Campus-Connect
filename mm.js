...
app.get('/int', (req, res) => {
  if(!req.session.info){
    ...
    res.sendFile(path.join(__dirname , 'public' , 'int2.html'));
  }
});

app.post('/int', function (req, res) {
  const { interest, priority } = req.body;
  const username = req.session.username;
  const sql = 'SELECT userid FROM users WHERE username = ?';
  connection.query(sql, [username], (err, userids) => {
      if (interest ) {
          const sql2 = 'INSERT INTO interests (userid, interest, priority) VALUES (?, ?, ?)';
          const userid = userids[0].userid;
          connection.query(sql2, [userid, interest.join(','), priority], (err, result) => {
              if (err) {
                  console.log(err);
                  res.redirect('/int');
              } else {
